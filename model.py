...
class Encoder_M(nn.Module):
    ...
    def forward(self, in_f, in_m, in_o):
        ...
        f = F.interpolate(in_f, scale_factor=ZOOM_FACTOR, mode='bilinear', align_corners=False)
        m = F.interpolate(m, scale_factor=ZOOM_FACTOR, mode='bilinear', align_corners=False)
        o = F.interpolate(o, scale_factor=ZOOM_FACTOR, mode='bilinear', align_corners=False)
...
class Decoder(nn.Module):
    def __init__(self, mdim):
        ...
        self.RF2 = Refine(256, mdim) # 1/4 -> 1/2
        #self.RF1 = Refine(64, mdim)
        #self.RF0 = Refine(3, mdim)

        self.pred2 = nn.Conv2d(mdim, 2, kernel_size=(3,3), padding=(1,1), stride=1)
   

    def forward(self, r4, r3, r2, r1, r0):
        ...
        #p = self.pred2(F.relu(m0))
        p = self.pred2(F.relu(m2))
        
        p = F.interpolate(p, scale_factor=4/ZOOM_FACTOR, mode='bilinear', align_corners=False)
        return p #, p2, p3, p4
...
class Memory(nn.Module):
    ...
    def forward(self, m_in, m_out, q_in, q_out, an_mask=None):  # m_in: o,c,t,h,w
        ...
        #mi = mi.to(torch.float16)
        #qi = qi.to(torch.float16)
        ...
        if activate_masks:
            if an_mask is None:
                ...
                an_mask = an_mask.cuda()
                an_mask = an_mask.view(1, T*H*W, 1)

                p_mean = torch.max(p, 1, keepdim=True)[0]
                p_exp = torch.exp(p - p_mean)
                p_exp = p_exp * an_mask
        ...
        #mem = mem.to(torch.device('cuda:0'))
...
class STM(nn.Module):
    ...
    def memorize(self, frame, masks, num_objects): 
        ...
        (frame, masks), pad = pad_divide_by([frame, masks], 8, (frame.size()[2], frame.size()[3]))
    ...
    def segment(self, frame, keys, values, num_objects, an_mask=None): 
        num_objects = num_objects[0].item()
        _, K, keydim, T, H, W = keys.shape # B = 1
        # pad
        [frame], pad = pad_divide_by([frame], 8, (frame.size()[2], frame.size()[3]))

        r4, r3, r2, c1, f = self.Encoder_Q(frame)
        ...
        c1e = c1.expand(num_objects,-1,-1,-1)
        fe = f.expand(num_objects,-1,-1,-1)
        
        # memory select kv:(1, K, C, T, H, W)
        m4, viz = self.Memory(keys[0,1:num_objects+1], values[0,1:num_objects+1], k4e, v4e, an_mask=an_mask)
        logits = self.Decoder(m4, r3e, r2e, c1e, fe)
    ...
    def changeMeanStd(mean, std):
        self.Encoder_M.changeMeanStd(mean, std)
        self.Encoder_M.changeMeanStd(mean, std)
